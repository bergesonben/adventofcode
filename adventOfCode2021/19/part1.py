...
def writeToCache(scanners):
    ...
    for scanner in scanners:
        relationshipSet = relationshipSet.union(scanner.relationships)
...
def getNumberOfBeacons(scanners):
    if USE_CACHE:
        ...
        for i, scanner1 in enumerate(scanners[:-1]):
            for j, scanner2 in enumerate(scanners[i+1:]):
...
def main(): 
    ...
    print('total number of beacons: ' + str(getNumberOfBeacons(scanners)))        
    print('num of relationships: ' + str(relationshipCounter))
